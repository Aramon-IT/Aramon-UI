...
export function GET() {
  ...
  const body = [`# Aramon UI`, ``, `Canonical design system for Aramon products and AI agents.`, ``, `## Start`, `- Installation: https://ui.aramon.ma/installation/`, `- Full agent reference: https://ui.aramon.ma/llms-full.txt`, `- Machine catalog: https://ui.aramon.ma/catalog.json`, `- Registry: https://ui.aramon.ma/r`, ``, `## Contract`, `- Use data-aramon-theme="light|dark"; light is the default.`, `- Lamp is the only primary action treatment.`, `- Medium and Material Background are sparse earned surfaces.`, `- Use logical CSS for EN/FR/AR and RTL.`, `- Respect keyboard, reduced motion, Save Data, and 44px targets.`, ``, `## Components`, ...catalog.map((entry) => `- ${entry.name}: https://ui.aramon.ma/components/${entry.slug}/index.md`)].join("\n");
