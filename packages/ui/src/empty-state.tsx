  ...
  return (
    <section className={cn("grid min-h-64 place-items-center border-y border-[var(--aramon-hairline)] px-6 py-12 text-center", className)} {...props}>
      <div className="max-w-sm">
        {icon ? <div aria-hidden="true" className="mx-auto mb-5 grid size-11 place-items-center rounded-full border border-[var(--aramon-hairline-strong)] bg-[var(--aramon-well)] text-aramon-ink-2">{icon}</div> : null}
        <h3 className="m-0 font-serif text-2xl font-normal tracking-[-.035em] text-aramon-ink">{title}</h3>
