...
export function Avatar({ alt, className, fallback, presence, ref, size = "default", src, ...props }: AvatarProps) {
  return (
    <span ref={ref} className={cn("relative inline-grid shrink-0 place-items-center rounded-full border border-[var(--aramon-hairline-strong)] bg-[var(--aramon-well)] font-mono text-aramon-ink-2", sizes[size], className)} {...props}>
      ...
      {presence ? <i aria-label={presence} className={cn("absolute bottom-0 end-0 size-[22%] min-h-2 min-w-2 rounded-full ring-2 ring-[var(--aramon-desk)]", presenceColors[presence])} /> : null}
    </span>
  );
}
